...
`Component---*.md`), that is the **source of truth** — it is regenerated by
CI on every push to the default branch. The llm-wiki instance's
`raw/arch/<project>/` (rig.json, model.c4) is a **fallback** that may be
