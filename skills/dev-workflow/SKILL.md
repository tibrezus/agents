---
name: dev-workflow
description: Enforce branch-based development on a project — every change flows issue → branch → milestone → green CI → merged PR, never directly on the default branch. Multi-platform (GitHub via gh, Forgejo/Codeberg via fj + REST). Includes an `adopt` command that injects or updates the workflow mandate into a project's AGENTS.md so the rule is always-on for every session. Use when about to implement a feature or fix in a project that follows this workflow, when creating issues/branches/PRs, when watching CI before merge, or when asked to set up / apply / propagate / change the development workflow in a repo's AGENTS.md.
---
...
4. **Change made on the branch** — commits reference the issue
   (`Refs #<n>` / `Fixes #<n>`).
5. **PR open** against the default branch.
6. **CI green** — every check succeeds. Red CI is fixed on the branch and
   re-pushed; it is **never** merged red.
7. **Merged** — only now does the default branch move. Branch deleted, issue
   closed.
...
- Auto-detects platform, default branch, and CI-watch command from the repo.
...
3. **Assign a milestone** (convention from the project's AGENTS.md):
   ...
   ```
4. **Make the change** on the branch. Commit with `Refs #$ISSUE` (or `Fixes #$ISSUE`).
...
## Relationship to fork-maintenance
...
For forked repos both apply: this skill governs your own feature branches;
fork-maintenance governs the upstream-sync PRs. Both share the CI-watch
pattern (`gh pr checks --watch` / commit status polling).
...
- [ ] CI green on the PR (watched, not assumed)
